 ...
 //  Definition for a binary tree node.
  public class TreeNode {
      ...
      public TreeNode(int val=0, TreeNode left=null, TreeNode right=null) {
        ...
        {
            HashSet<int> nodes2 = new HashSet<int>();
            HashSet<int> hasParent = new HashSet<int>();
            Dictionary<int, TreeNode> nodes = new Dictionary<int, TreeNode>();
            int p, c;
            bool isLeft;
            TreeNode n;
            foreach(int[] t in descriptions)
            {
                p = t[0];
                c = t[1];
                isLeft = (t[2] == 1);
                ...
                else
                {
                    if (isLeft)
                        n.left = nodes[c];
                    else n.right = nodes[c];
                }
                hasParent.Add(c);
            }

            foreach (int x in nodes.Keys)
                if (!hasParent.Contains(x))
                    return nodes[x];
            return null;
        }
        ...
        public static IList<string> CellsInRange(string s)
        {
            ...
            for (int i = 0; i < maxI; i++)
                for (int j = 0; j < maxJ; j++)       
                {
                    letter =(char) (s[0]+i);
                    digit = (char)(s[1]+j);
                    list.Add(String.Format("{0}{1}", letter, digit));
                }
            return list;
        }
    }
